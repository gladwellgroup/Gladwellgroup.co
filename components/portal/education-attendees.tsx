...
interface EducationAttendeesProps {
  sessionId: string
  attendees: AttendeeData[]
  /** El reenvío solo tiene sentido una vez el entregable ya se envió: antes
   *  de eso el envío lo hace "Dar visto bueno", que además reintenta solo a
   *  los pendientes si el primer intento quedó a medias. */
  deliverySent: boolean
  disabled?: boolean
}

export function EducationAttendees({
  sessionId,
  attendees,
  deliverySent,
  disabled,
}: EducationAttendeesProps) {
  ...
  const [confirmDelete, setConfirmDelete] = useState<AttendeeData | null>(null)
  ...
  async function handleResend() {
    ...
    try {
      const res = await fetch('/api/education/deliverables/resend-attendees', {
        method: 'POST',
  ...
  async function handleImport() {
    ...
    try {
      const res = await fetch('/api/education/attendees/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDelete(attendee: AttendeeData) {
    setConfirmDelete(null)
    setError(null)

    try {
      const res = await fetch(`/api/education/attendees/${attendee.id}`, {
        method: 'DELETE',
      })
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
    ...
    <div className="space-y-4">
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-xs">
                Debe tener una columna de correo y, idealmente, una de nombre.
              </p>
            </div>
          </button>
          <input
      ...
      {attendees.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Aún no hay asistentes en esta sesión.
        </p>
      ) : (
